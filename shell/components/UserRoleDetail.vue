...
export default {
  ...
  computed: {
    ...
    canEditGlobalRole() {
      const schema = !!this.$store.getters[`management/schemaFor`](MANAGEMENT.GLOBAL_ROLE);

      return this.canSeeGlobalRoles && this.canSeeRoleTemplates && (schema?.resourceMethods.includes('PUT') || schema.resourceMethods.includes('blocked-PUT'));
    },
    canEditClusterRoles() {
      const schema = !!this.$store.getters[`management/schemaFor`](MANAGEMENT.CLUSTER_ROLE_TEMPLATE_BINDING);

      return schema && this.canSeeRoleTemplates && (schema?.resourceMethods.includes('PUT') || schema.resourceMethods.includes('blocked-PUT'));
    },
    canEditProjectRoles() {
      const schema = !!this.$store.getters[`management/schemaFor`](MANAGEMENT.PROJECT_ROLE_TEMPLATE_BINDING);
  ...
  methods: {
    ...
    addClusterRole() {
      this.$store.dispatch('cluster/promptModal', {
        component:      'AddClusterMemberDialog',
        componentProps: {
          saveInModal: true,
    ...
    onAdd(bindings) {
      bindings.forEach(b => b.save());
    },

    // save bindings when form using this component is saved
    save() {}
  }
      ...
      >
        ...
        <template v-else>
          <GlobalRoleBindings
            ...
            :real-mode="realMode"
            type="user"
            @hasChanges="$emit('hasChanges', $event)"
            @canLogIn="$emit('canLogIn', $event)"
          />
        </template>
      </Tab>
      <Tab
