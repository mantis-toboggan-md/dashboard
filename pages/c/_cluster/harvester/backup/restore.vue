...
<template>
  <div id="restore">
    ...
    <LabeledSelect v-model="backupName" :mode="mode" class="mb-20" :label="t('harvester.backup.restore.backup')" :options="backupOption" />

    <LabeledSelect v-if="!restoreNewVm" v-model="deletionPolicy" :mode="mode" :label="t('harvester.backup.restore.deletePreviousVolumes')" :options="deletionPolicyOption" />

    <Footer :mode="mode" :errors="errors" @save="saveRestore" @done="done" />
  </div>
</template>
